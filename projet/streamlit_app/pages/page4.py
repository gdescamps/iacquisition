...
uploaded_file_page4 = st.file_uploader("Upload a PDF file", type="pdf",
...
if uploaded_file_page4 is not None:
    if st.session_state.get('page4_response_data') is None:
        response, res = launch_ocr_request(filename=uploaded_file_page4.name, file_content=uploaded_file_page4, file_type=uploaded_file_page4.type)
        ...
        if "messages" not in st.session_state:
            st.session_state.messages = [{"role": "system", "content": os.getenv("CHATBOT_JOBDESC_SYSTEM_KNOWLEDGE")}]
